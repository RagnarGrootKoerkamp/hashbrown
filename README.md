# HashBrown with prefetching

This fork adds two instructions: `prefetch(key)->u64` that prefetches the control bytes and
data line for the location where a key maps and returns the hash of the key, and
`contains_with_hash(key, hash)` that checks if the given key is present, reusing
the hash returned by prefetch.
